...
LowPassFilter::LowPassFilter(std::size_t windowSize)
...
FilterState LowPassFilter::apply(Waveform* sound)
{
    ...
    for (std::size_t index = 0; index < source.size(); ++index) {
        ...
        for (std::size_t offset = 0; offset < _windowSize; ++offset) {
            ...
            const std::size_t clampedIndex = static_cast<std::size_t>(
        ...
        sound->getSamples()[index] = clampSample(
            static_cast<double>(sum) / _windowSize);
    }

    return FilterState::Success;
}

std::size_t LowPassFilter::getWindowSize() const
{
    return _windowSize;
}
