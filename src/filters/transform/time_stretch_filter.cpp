...
FilterState TimeStretchFilter::apply(Waveform* sound)
{
    ...
    const double requestedSize =
        samples.size() * _factor;  // new count of samples after stretching
    ...
    std::vector<std::int16_t> stretched(
        newSize);  // create new vector for stretched samples

    for(std::size_t index = 0; index < newSize;
        ++index)  // fill the scretched vector
    {
        const double position =
            index / _factor;  // new position in the original samples vector
