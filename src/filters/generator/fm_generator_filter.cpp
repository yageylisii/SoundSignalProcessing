...
FilterState FmGeneratorFilter::apply(Waveform* sound)
{
    ...
    if(!std::isfinite(_amplitude) || !std::isfinite(_carrierHz) ||
       !std::isfinite(_modulationHz) || !std::isfinite(_deviationHz) ||
       _amplitude < 0.0 || _amplitude > 1.0 || _carrierHz < 0.0 ||
       _modulationHz <= 0.0 || _deviationHz < 0.0 ||
    ...
    for(std::size_t index = 0; index < sampleCount; ++index)
    {
        ...
        const double phase = 2.0 * Pi * _carrierHz * time +
                             (_deviationHz / _modulationHz) *
                                 std::sin(2.0 * Pi * _modulationHz * time);
        sound->getSamples()[index] =
            clampSample(_amplitude * MaximumAmplitude * std::sin(phase));
    }
