...
WavWriter::Result WavWriter::write(const std::string& fileName,
                                   const Waveform& waveform)
{
    _errorMessage.clear();
    // check correct of waveform parameters and samples count
    if(waveform.getSampleRate() != 44100 || waveform.getChannelCount() != 1 ||
       waveform.getBitsPerSample() != 16 ||
       waveform.getSamples().size() % waveform.getChannelCount() != 0)
    {
        _errorMessage = "Only mono 44100 Hz 16-bit PCM audio can be written";
        return Result::UnsupportedFormat;
    }

    // common size of samples in bytes
    const std::uint64_t dataSize64 =
        waveform.getSamples().size() * sizeof(std::int16_t);
    // count bytes for one moment of time (for all channels)
    const std::uint64_t blockAlign64 =
        waveform.getChannelCount() * sizeof(std::int16_t);
    // bytes per second for the whole audio stream
    const std::uint64_t byteRate64 =
        static_cast<std::uint64_t>(waveform.getSampleRate()) * blockAlign64;

    // check correct of data from waveform
    ...
    // open in binary mode for correct writing of bytes
    ...
    // fill the WAV file byte by byte
    const std::uint32_t dataSize = static_cast<std::uint32_t>(dataSize64);
    const RiffHeader riffHeader = {
        {'R', 'I', 'F', 'F'}, 36 + dataSize, {'W', 'A', 'V', 'E'}};
    const ChunkHeader fmtHeader = {{'f', 'm', 't', ' '}, sizeof(FmtChunkData)};
    const FmtChunkData fmtData = {1,
                                  waveform.getChannelCount(),
                                  waveform.getSampleRate(),
                                  static_cast<std::uint32_t>(byteRate64),
                                  static_cast<std::uint16_t>(blockAlign64),
                                  waveform.getBitsPerSample()}; // fmt chunk data
    const ChunkHeader dataHeader = {{'d', 'a', 't', 'a'}, dataSize}; // data chunk
    // fill and check
    if(!writeObject(output, riffHeader) || !writeObject(output, fmtHeader) ||
       !writeObject(output, fmtData) || !writeObject(output, dataHeader) ||
       (dataSize != 0 && !output.write(reinterpret_cast<const char*>(
                                           waveform.getSamples().data()),
...
const std::string& WavWriter::getErrorMessage() const { return _errorMessage; }
